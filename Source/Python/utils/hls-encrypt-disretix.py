...
def main():
    # parse options
    global Options
    parser = OptionParser(usage="%prog [options] <buydrm-user-key> <input-m3u8-file> <output_dir>",
                          description="""<input-m3u8-file> is the path to a source HLS plalist file. 
                                      <output_dir> - directory to write protected files to.\n
                                      <buydrm-user-key> - your user key from keyos system.\n"""
                                      """Version """ + VERSION)
    ...
    if len(args) == 0:
        parser.print_help()
        sys.exit(1)
    buydrm_user_key = args[0]
    playlist = m3u8.HLSPlaylist(args[1])
    encryption_key, prHeader = buydrm.GetBuyDRMKey(buydrm_user_key)
    kid_hex, key_hex = encryption_key.split(':')
    kid = uuid.UUID(kid_hex)
    playlist.addDiscretixPRHeader(base64.b64encode(kid.bytes), prHeader)
    playlist.protectSegments(key_hex, base64.b64encode(kid.bytes), args[2])
    playlist.storeEncrypted(args[2])   
